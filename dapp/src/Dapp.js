...
class Dapp extends Component {
  ...
  */
  ...
  handleChangeCourt = (event) => {
    event.preventDefault()
    this.setState({court: event.target.value});
    if ('' !== event.target.value && !this.isAddress(event.target.value)) {
      this.setState({errCourt: true});
    } else {
      this.setState({errCourt: false});
    }
  }
  ...
  handleChangeTimeToReact = (event) => {
    event.preventDefault()
    this.setState({timeToReact: event.target.value});
