...
library RLPReader {
    ...
    /// @notice Copy RLP value, padded to `maxLen`
    function _copyPadded(
        ...
        require(_length <= maxLen, "RLPReader: pad overflow");
        ...
        uint256 src = MemoryPointer.unwrap(_src) + _offset;
        assembly {
            let padLen := sub(maxLen, _length)
            let dest := add(add(out, 32), padLen)
            let i := 0
            for {
    ...
    function readUint256(RLPItem memory _in) internal pure returns (uint256) {
        ...
        require(itemLength <= 32, "RLPReader: number too big");

        return
            uint256(bytes32(_copyPadded(_in.ptr, itemOffset, itemLength, 32)));
    }
}