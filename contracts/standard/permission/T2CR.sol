 ...
 *  NOTE: The differences from this contract to ArbitrableTokenList are the following:
 *  - In the case of an unsuccessful appeal crowdfunding, contributors are reimbursed rather than lose their contribution to the winner.
 *  - getTokenInfo method has been renamed to getItemInfo.
 *  - queryTokens method has been renamed to queryItems.
 */
contract T2CR is PermissionInterface, Arbitrable {
     ...
     */
    function queryItems(bytes32 _cursor, uint _count, bool[8] _filter, bool _oldestFirst, address _tokenAddr)
        external
        view
     ...
     */
    function getItemInfo(bytes32 _tokenID)
        external
        view
        returns (
            string name,
