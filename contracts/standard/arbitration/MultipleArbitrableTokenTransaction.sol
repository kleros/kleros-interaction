 ...
 *  Parties are identified as "receiver" and "sender".
 */
...
contract MultipleArbitrableTokenTransaction {
    ...
    enum Status {NoDispute, WaitingReceiver, WaitingSender, DisputeCreated, Resolved}
    ...
    function pay(uint _transactionID, uint _amount) public {
        ...
        require(token.transfer(transaction.receiver, _amount) != false, "The `transfer` function must not returns `false`.");
    }
    ...
    function reimburse(uint _transactionID, uint _amountReimbursed) public {
        ...
        token.transfer(transaction.sender, _amountReimbursed);
    }
    ...
    function executeTransaction(uint _transactionID) public {
        ...
        token.transfer(transaction.receiver, amount);
    }
     ...
     *  Note that this function mirrors payArbitrationFeeByReceiver.
     *  @param _transactionID The index of the transaction.
     */
     ...
     *  @param _transactionID The index of the transaction.
     *  @param _ruling Ruling given by the arbitrator. 1: Reimburse the sender. 2: Pay the receiver.
     */
    function executeRuling(uint _transactionID, uint _ruling) internal {
        ...
        if (_ruling == uint(RulingOptions.ReceiverWins)) {
            ...
            token.transfer(transaction.receiver, amount);
        } else if (_ruling == uint(RulingOptions.SenderWins)) {
            transaction.sender.send(senderFee);
            token.transfer(transaction.sender, amount);
        } else {
            uint split_arbitration_fee = (receiverFee + senderFee - transaction.arbitrationCost) / 2;
            ...
            token.transfer(transaction.sender, amount / 2);
            token.transfer(transaction.receiver, amount / 2);
        }
    }
    ...
    /** @dev Get IDs for transactions where the specified address is the sender and/or the receiver.
    ...
    function getTransactionIDsByAddress(address _address) public view returns (uint[] transactionIDs) {
        ...
        for (uint i = 0; i < transactions.length; i++) {
            if (transactions[i].receiver == _address || transactions[i].sender == _address)
                count++;
        }
        ...
        for (uint j = 0; j < transactions.length; j++) {
            if (transactions[j].receiver == _address || transactions[j].sender == _address)
