...
contract('AppealableArbitrator', function (accounts) {
    ...
    const choices = 2
    const arbitratorExtraData = 0xfa
    ...
    it('Should set the correct values', async () => {
        ...
        assert.equal(await appealableArbitrator.arbitrationCost(0xfa), arbitrationFee)
        assert.equal(await appealableArbitrator.arbitrator(), centralizedArbitrator.address)
        assert.equal(await appealableArbitrator.arbitratorExtraData(), arbitratorExtraData)
    })
    ...
    it('Non-owner cant rule none-appealed disputes', async () => {
        ...
        await appealableArbitrator.createDispute(choices, arbitratorExtraData, { value: arbitrationFee })
    ...
    it('Ruling should set correct values in none-appealed disputes and create AppealPossible event', async () => {
        ...
        assert.equal(dispute[2].toNumber(), 2, 'Arbitration fee not set up properly')
        ...
        assert.equal(tx.logs[0].event, 'AppealPossible', 'The event has not been created')
        assert.equal(tx.logs[0].args._disputeID.toNumber(), 0, 'The event has wrong dispute ID')
        assert.equal(tx.logs[0].args._arbitrable, appealable, 'The event has wrong arbitrable address')
    ...
    it('Shouldnt allow to make final ruling in non-appealed dispute before timeout', async () => {
        ...
        increaseTime(1)
        await expectThrow(
            appealableArbitrator.giveRuling(0, 1)
        )
    ...
    it('Shouldnt be possible to pay less than appeal fee', async () => {
        ...
        await appealableArbitrator.createDispute(choices, arbitratorExtraData, { value: arbitrationFee })
        await expectThrow(
            appealableArbitrator.appeal(0, arbitratorExtraData, {from: appealable, value: 20})
        )
        await appealableArbitrator.giveRuling(0, 1)
        await expectThrow(
            appealableArbitrator.appeal(0, arbitratorExtraData, {from: appealable, value: arbitrationFee - 1})
        )   
    ...
    it('Should set correct values after appeal is done', async () => {
        ...
        await centralizedArbitrator.createDispute(choices, arbitratorExtraData, { value: arbitrationFee })
               
        await appealableArbitrator.createDispute(choices, arbitratorExtraData, { value: arbitrationFee })
        await appealableArbitrator.createDispute(choices, arbitratorExtraData, { value: arbitrationFee })
        await appealableArbitrator.createDispute(choices, arbitratorExtraData, { value: arbitrationFee })
        await appealableArbitrator.giveRuling(2, 1)
        await appealableArbitrator.appeal(2, arbitratorExtraData, {value: arbitrationFee})

        const appealDispute = await appealableArbitrator.appealDisputes(2)

        assert.equal(appealDispute[1], centralizedArbitrator.address, 'Appeal has wrong arbitrator')
        assert.equal(appealDispute[2].toNumber(), 1, 'Appeal has wrong ID')
        
        const appealDisputeIDsToDisputeIDs = await appealableArbitrator.appealDisputeIDsToDisputeIDs(1)
        assert.equal(appealDisputeIDsToDisputeIDs.toNumber(), 2, 'Dispute ID has incorrect appeal ID')

        const dispute = await centralizedArbitrator.disputes(1)
        assert.equal(dispute[0], appealableArbitrator.address, 'Arbitrable not set up properly')
    ...
    it('Should return correct appeal cost', async () => {
        ...
        assert.equal(appealCost.toNumber(), NOT_PAYABLE_VALUE, 'Incorrect appeal cost for dispute before ruling')
        
        await appealableArbitrator.giveRuling(0, 1)
        appealCost = await appealableArbitrator.appealCost(0, arbitratorExtraData)
        assert.equal(appealCost.toNumber(), 3, 'Incorrect appeal cost for dispute after ruling')
        
        await appealableArbitrator.appeal(0, arbitratorExtraData, {from: appealable, value: 3})
        appealCost = await appealableArbitrator.appealCost(0, arbitratorExtraData)
        appealCost2 = await centralizedArbitrator.appealCost(0, arbitratorExtraData)
        assert.equal(appealCost.toNumber(), appealCost2.toNumber(), 'Incorrect appeal cost after appeal was made')
    ...
    it('Should return correct status', async () => {
        ...
        await appealableArbitrator.appeal(0, arbitratorExtraData, {from: appealable, value: arbitrationFee})
        dispute = await centralizedArbitrator.disputes(0)
        status = await appealableArbitrator.disputeStatus(0)
        assert.equal(status.toNumber(), dispute[4].toNumber(), 'Incorrect status after appeal')
    ...
    it('Should set correct values in disputes after final ruling after appeal', async () => {
        ...
        assert.equal(disputeAppeal[3].toNumber(), disputeArbitrator[3].toNumber(), 'Arbitrator and arbitrable contracts have different rulings')
        assert.equal(disputeAppeal[4].toNumber(), 2, 'Incorrect status in arbitrable contract')
        assert.equal(disputeArbitrator[4].toNumber(), 2, 'Incorrect status in arbitrator contract')
    ...
    it('Should get correct latest AppealID', async () => {
        ...
        assert.equal(appealID.toNumber(), 2, 'Incorrect appealID before appeal was made')
        
        await appealableArbitrator.giveRuling(2, 1)
        await appealableArbitrator.appeal(2, arbitratorExtraData, {value: arbitrationFee})

        appealID = await appealableArbitrator.getAppealDisputeID(2)
        assert.equal(appealID.toNumber(), 1, 'Incorrect appealID after appeal was made')
