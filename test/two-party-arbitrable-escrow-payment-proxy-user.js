...
const TwoPartyArbitrableEscrowPaymentProxyUser = artifacts.require(
  './standard/arbitration/composed-arbitrable/example/TwoPartyArbitrableEscrowPaymentProxyUser.sol'
)
...
contract('TwoPartyArbitrableEscrowPaymentProxyUser', accounts =>
  it('Should function as an arbitrable escrow service with crowdinsured fee payments.', async () => {
    // Deploy contracts
    const governor = accounts[0]
    const stake = 10
    const twoTimesStake = stake * 2
    const multiPartyInsurableArbitrableAgreementsProxy = await MultiPartyInsurableArbitrableAgreementsProxy.new(
      governor, // _arbitrator
      null, // _arbitratorExtraData
      governor, // _feeGovernor
      stake // _stake
    )
    const twoPartyArbitrableEscrowPaymentProxyUser = await TwoPartyArbitrableEscrowPaymentProxyUser.new(
      multiPartyInsurableArbitrableAgreementsProxy.address // _arbitrableProxy
    )
    ...
    await checkOnlyByGovernor(
      multiPartyInsurableArbitrableAgreementsProxy.feeGovernor,
      governor,
      multiPartyInsurableArbitrableAgreementsProxy.changeFeeGovernor,
      accounts[1],
      accounts[2],
      accounts[1]
    )
    await checkOnlyByGovernor(
      multiPartyInsurableArbitrableAgreementsProxy.stake,
      stake,
      multiPartyInsurableArbitrableAgreementsProxy.changeStake,
      0,
      accounts[2]
    )
    ...
    for (const payment of payments) {
      ...
      await twoPartyArbitrableEscrowPaymentProxyUser.createPayment(
        payment.ID,
        evidence,
        receiver,
        payment.arbitrationFeesWaitingTime,
    ...
    await multiPartyInsurableArbitrableAgreementsProxy.submitEvidence(
      appealTimeOutPayment.ID,
      evidence
    )
    await expectThrow(
    ...
    const rulingDisputeID = Number(
      (await multiPartyInsurableArbitrableAgreementsProxy.agreements(
        appealTimeOutPayment.ID
      ))[5]
    )
    await expectThrow(
    ...
    for (const payment of payments)
      for (let i = 0; i < payment.contributionsPerSide.length; i++)
        if (i > 0 || (i === 0 && payment.contributionsPerSide.length === 1))
          await multiPartyInsurableArbitrableAgreementsProxy.withdrawReward(
            payment.ID,
            i
          )
